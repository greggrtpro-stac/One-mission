...
/** Le strict nécessaire sur l'utilisateur pour calculer ses stats. */
...
/**
 * Calcule toutes les statistiques de profil d'un joueur (page Profil et
 * profil public). Le rang suit le même tri et le même filtre de visibilité
 * que le classement.
 */
export async function computeProfileStats(user: StatsSubject): Promise<ProfileStats> {
    ...
    prisma.user.count({
      where: {
        ...
        OR: [
          ...
          { totalXp: user.totalXp, createdAt: { lt: user.createdAt } },
        ],
      },
    }),
  ...
  const daysSinceCreation = Math.max(
    1,
    Math.ceil((now.getTime() - user.createdAt.getTime()) / DAY_MS),
  )
